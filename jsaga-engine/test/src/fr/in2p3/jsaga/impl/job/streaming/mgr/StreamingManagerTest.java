...
public class StreamingManagerTest {
    ...
    public void modifyDescriptionTwoPhase() throws Exception {
        ...
        StreamingManagerThroughSandboxTwoPhase manager = new StreamingManagerThroughSandboxTwoPhase(sja, "456");
        assertEquals("worker-456.txt", manager.getWorker("txt"));
        // modify description and check
        JobDescription newJobDesc = manager.modifyJobDescription(new SAGAJobDescriptionImpl());
        assertEquals(JobDescription.FALSE, newJobDesc.getAttribute(JobDescription.INTERACTIVE));
        String[] transfers = newJobDesc.getVectorAttribute(JobDescription.FILETRANSFER);
        assertEquals(3, transfers.length);
        assertEquals(LocalFileFactory.getLocalInputFile("456").getAbsolutePath() + " > " + manager.getWorker("input"),
                transfers[0]);
        assertEquals(LocalFileFactory.getLocalOutputFile("456").getAbsolutePath() + " < " + manager.getWorker("output"),
                transfers[1]);
        assertEquals(LocalFileFactory.getLocalErrorFile("456").getAbsolutePath() + " < " + manager.getWorker("error"),
                transfers[2]);
        assertEquals(manager.getWorker("input"), newJobDesc.getAttribute(JobDescription.INPUT));
        assertEquals(manager.getWorker("output"), newJobDesc.getAttribute(JobDescription.OUTPUT));
        assertEquals(manager.getWorker("error"), newJobDesc.getAttribute(JobDescription.ERROR));
        // touch transfer files
        LocalFileFactory.getLocalInputFile("456").createNewFile();
        LocalFileFactory.getLocalOutputFile("456").createNewFile();
        LocalFileFactory.getLocalErrorFile("456").createNewFile();
        // cleanup
        manager.cleanup(null, "456");
        // check
        assertFalse(LocalFileFactory.getLocalInputFile("456").exists());
        assertFalse(LocalFileFactory.getLocalOutputFile("456").exists());
        assertFalse(LocalFileFactory.getLocalErrorFile("456").exists());
    }
}
