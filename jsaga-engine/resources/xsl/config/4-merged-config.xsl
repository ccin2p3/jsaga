...
<xsl:stylesheet version="1.0"
    ...
    <xsl:param name="jsaga.universe.ignore.missing.adaptor" select="'true'"/>
    ...
    <xsl:template match="cfg:context">
        ...
        <xsl:choose>
            <xsl:when test="not($desc) and $jsaga.universe.ignore.missing.adaptor='true'">
                <xsl:message terminate="no">Missing plugin for: <xsl:value-of select="@type"/></xsl:message>
            </xsl:when>
            <xsl:when test="not($desc) and $jsaga.universe.ignore.missing.adaptor='false'">
