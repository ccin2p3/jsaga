...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="jsdl:JobDescription">
		...
		<xsl:choose>
    		...
    		<xsl:otherwise>
    			<xsl:text>DefaultJob</xsl:text><xsl:value-of select="$UniqId"></xsl:value-of>
    		</xsl:otherwise>
       </xsl:choose>
   ...
   <xsl:for-each select="jsdl:Application/jsdl-posix:POSIXApplication">
		...
		<xsl:text>&#xa;</xsl:text>
		<xsl:text>init: put </xsl:text>
		<xsl:value-of select="$UniqId"/><xsl:text>-</xsl:text><xsl:value-of select="position()"/><xsl:text>.sh </xsl:text><xsl:value-of select="$UniqId"/> <xsl:text>-</xsl:text><xsl:value-of select="position()"/><xsl:text>.sh</xsl:text><xsl:text>&#xa;</xsl:text>
		<xsl:for-each select="/jsdl:JobDefinition/jsdl:JobDescription/jsdl:DataStaging">
		 	<xsl:if test="jsdl:Source"><xsl:text>put </xsl:text><xsl:value-of select="jsdl:FileName/text()"/><xsl:text> </xsl:text><xsl:value-of select="jsdl:FileName/text()"/><xsl:text>&#xa;</xsl:text></xsl:if>
    	</xsl:for-each>
	    ...
	    <xsl:choose>
			<xsl:when test="(jsdl-posix:Argument)or(jsdl-posix:Input) ">
				<xsl:for-each select="/jsdl:JobDefinition/jsdl:JobDescription/jsdl:DataStaging[jsdl:Target][not(contains($SupportedProtocols,concat('/',substring-before(jsdl:Target/jsdl:URI/text(),'://'),'/')))]">
					<xsl:text>get</xsl:text><xsl:value-of select="jsdl:FileName/text()"/> <xsl:text> </xsl:text> <xsl:value-of select="jsdl:FileName/text()"/><xsl:text>&#xa;</xsl:text>
				</xsl:for-each>
 			</xsl:when>
		</xsl:choose>
    </xsl:for-each>			
    ...
    <xsl:if test="jsdl:DataStaging/jsdl:DeleteOnTermination[text()='True']">
      		 <xsl:text>&#xa;</xsl:text>
      	 	 <xsl:text disable-output-escaping="yes"><![CDATA[<]]>CleanUp<![CDATA[>]]>True<![CDATA[</]]>CleanUp<![CDATA[>]]>  </xsl:text> 
    </xsl:if>  
    ...
    <xsl:if test="jsdl:DataStaging">
					...
					<![CDATA[<]]>UploadURL<![CDATA[>]]><xsl:value-of select="$SANDBOX_BASE_URI"/>upload/<![CDATA[</]]>UploadURL<![CDATA[>]]>
	</xsl:if>
