...
public class VOMSMyProxySecurityCredential extends VOMSSecurityCredential {
        ...
        if (Context.LIFETIME.equals(key)) {
            ...
            try {
              ...
              // the genuine lifetime is the one on the server (set by the user config)
                timeleft = timeleft + UDuration.toInt(this._genuineLifeTime)-UDuration.toInt(VOMSMyProxySecurityAdaptor.LOCAL_LIFETIME);
    ...
    public void dump(PrintStream out) throws Exception {
        ...
        // the genuine lifetime is the one on the server (set by the user config)
        out.println("  timeleft : "+Util.formatTimeSec(globusProxy.getTimeLeft()+UDuration.toInt(this._genuineLifeTime)-UDuration.toInt(VOMSMyProxySecurityAdaptor.LOCAL_LIFETIME)));
