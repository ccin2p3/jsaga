...
<!-- 		<xsl:text> &lt;</xsl:text><xsl:value-of select="$RootDir"/><xsl:text>/</xsl:text><xsl:value-of select="$UniqId"/><xsl:text>.in</xsl:text>
 ...
 -->		<xsl:text> &amp; &apos;; </xsl:text>
		...
		<!-- Store the PID -->
		<xsl:text>echo $MYPID &gt; $HOME/</xsl:text><xsl:value-of select="$RootDir"/><xsl:text>/</xsl:text><xsl:value-of select="$UniqId"/><xsl:text>.pid ;</xsl:text>
		<!-- Wait until process has finished -->
		<xsl:text>wait $MYPID; </xsl:text>
		<!-- Get return code -->
		<xsl:text>errcode=$?; </xsl:text>
		<!-- Store return code -->
		<xsl:text>echo $errcode &gt; $HOME/</xsl:text><xsl:value-of select="$RootDir"/><xsl:text>/</xsl:text><xsl:value-of select="$UniqId"/><xsl:text>.endcode ;</xsl:text>
		<!-- exit -->
