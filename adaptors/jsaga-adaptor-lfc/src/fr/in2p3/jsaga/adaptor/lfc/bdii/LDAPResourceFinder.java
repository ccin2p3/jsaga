...
package fr.in2p3.jsaga.adaptor.lfc.bdii;
...
public class LDAPResourceFinder {
	...
	private static Logger s_logger = Logger.getLogger(LDAPResourceFinder.class);
...
//		this.preferredSEID = (String) gatContext.getPreferences().get(GliteConstants.PREFERENCE_PREFERRED_SE_ID);
		this.preferredSEID = null;
		
		String ldapContact = null;;
		if (ldapResource == null) {
//			ldapContact = (String) gatContext.getPreferences().get(GliteConstants.PREFERENCE_BDII_URI);
			if (ldapContact == null) {
				ldapContact = DEFAULT_LDAP_SERVER_NAME + ":" + DEFAULT_LDAP_SERVER_PORT;
			}
		} else {
			final String host = ldapResource.getHost();
			int port = ldapResource.getPort();
			if(port == -1){
				port = DEFAULT_LDAP_SERVER_PORT;
			}
			if (host == null) {
	...
	private String getSafeStringAttr(SearchResult result, String name) throws NamingException {
	...
	public List<String> fetchWMSServers(final String voName) throws NamingException {
		...
		while (results.hasMore()) {
			...
			if (wmsServer == null) {
				wmsServer = getSafeStringAttr(result, "GlueServiceAccessPointURL");
			}
			if (wmsServer != null) {
				wmsServers.add(wmsServer);
				s_logger.info("Retrieved the following WMS server from LDAP: " + wmsServer);
			}
		}
	...
	public List<String> fetchCEs(final String voName) throws NamingException {
		...
		while (clusters.hasMore()) {
			...
			s_logger.info("Retrieved the following CE from LDAP: " + ceURL);
			if (ceURL != null)
				results.add(ceURL);
		}
	...
	public List<String> fetchLFCs(final String voName) throws NamingException {
		...
		String filter = "(&(objectClass~=GlueService)(GlueServiceType=lcg-file-catalog)(GlueServiceOwner=" + voName + "))";
		NamingEnumeration<SearchResult> searchResults = ctx.search(START_DN, filter, globalSearchControls);
		while (searchResults.hasMore()) {
			SearchResult result = searchResults.nextElement();
			String endpoint = getSafeStringAttr(result, "GlueServiceEndpoint");
			results.add(endpoint);
			s_logger.info("Retrieved the following LFC from LDAP: " + endpoint);
		}
		return results;
	}
	...
	public List<SEInfo> fetchSEs(final String voName, final long fileSize) throws NamingException {
		...
		findAndMoveSE(System.getenv("VO_" + voName.toUpperCase(Locale.ENGLISH) + "_DEFAULT_SE"), unsortedList, orderedList);
		Collections.shuffle(unsortedList);
		orderedList.addAll(unsortedList);
		return orderedList;
	}

	private void findAndMoveSE(String seid, List<SEInfo> sourceList, List<SEInfo> targetList) {
		if (seid == null) {
			return;
		}
	...
	private List<SEInfo> fetchAllSEs(final String voName, final long fileSize) throws NamingException {

		ArrayList<SEInfo> results = new ArrayList<SEInfo>();

		final String filter = "(&(objectClass~=GlueSA)(GlueSALocalID=" + voName + "))";
		final NamingEnumeration<SearchResult> searchResults = ctx.search(START_DN, filter, globalSearchControls);
		...
		while (searchResults.hasMore()) {
			...
			String space = getSafeStringAttr(result, "GlueSAStateAvailableSpace");

			if (seUniqueId != null) {
				boolean valid;
				valid = Long.parseLong(space) > fileSize;
				s_logger.debug("Found SE: " + seUniqueId + " with path " + path + " and space " + space + " Valid: " + valid);
				if (valid) {
