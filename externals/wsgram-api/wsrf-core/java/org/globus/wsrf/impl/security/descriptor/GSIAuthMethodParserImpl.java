...
public class GSIAuthMethodParserImpl extends ElementParser
    ...
    public AuthMethod getMethod() {
        if (protLevelParser.isPrivacy() && !protLevelParser.isIntegrity()) {
            return GSISecureConvAuthMethod.PRIVACY;
        } else if (
            protLevelParser.isIntegrity() && !protLevelParser.isPrivacy()
        ) {
            return GSISecureConvAuthMethod.INTEGRITY;
        } else {
            // either both true or both false
            return GSISecureConvAuthMethod.BOTH;
        }
    }
}
