...
public class Gram  {
    ...
    public static void renew(GramJob job, GSSCredential newCred,
        ...
        try {
            ...
            updateCallbackHandlerCredential(currentCred, newCred);
    ...
    protected static void updateCallbackHandlerCredential(GSSCredential currentCred, 
        GSSCredential newCred) {
	
	synchronized (callbackHandlers) {
	    CallbackHandler handler = (CallbackHandler) callbackHandlers.remove(currentCred);
	
	    if (handler != null) {
		handler.setCredentials(newCred);
	    
		callbackHandlers.put(newCred, handler);
    	    }
    	}
    }
    ...
    /**    */
    private static CallbackHandler initCallbackHandler(GSSCredential cred)
	throws GSSException, GramException {
	if (cred == null) {
	    throw new IllegalArgumentException("cred == null");
	}
