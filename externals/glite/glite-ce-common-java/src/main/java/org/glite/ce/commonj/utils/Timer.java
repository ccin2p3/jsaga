...
public class Timer {
   /**
     ...
     * activity), subsequent executions will be delayed as well. In the long
     * run, the frequency of execution will generally be slightly lower than the
     * reciprocal of the specified rate (assuming the system clock underlying
     * <tt>Object.wait(long)</tt> is accurate).
     * 
     * <p>
     * Fixed-delay execution is appropriate for recurring activities that
     ...
     * the reciprocal of the specified rate (assuming the system clock
     ...
     * with the specified rate, in milliseconds. If rate is positive, the task
     * is scheduled for repeated execution; if rate is zero, the task is
            ...
            try {
                ...
                if (taskFired) { // Task fired; run it, holding no locks
                    try {
                        ...
                        } else if (task.getExecutionType() == TimerTask.EXECUTION_TYPE.FIXED_DELAY_POST_EXECUTION) { // Repeating
                                                                                                                    // task,
                                                                                                                    // reschedule
                            synchronized (task.lock) {
                                task.setState(TimerTask.STATUS.SCHEDULED);
                        ...
                        logger.error("task " + task.getName() + " error: " + t.getMessage());
                    }
                }
