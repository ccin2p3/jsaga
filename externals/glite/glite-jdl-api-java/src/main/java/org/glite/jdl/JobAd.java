	...
	* @throws JobAdException - one or more values do not  match with jobad semantic rule*/
	...
	private void checkJobType(Vector vectType) throws Exception {  //TBD throws Exception fix me!!!
		...
		if(!hasAttribute(Jdl.JOBTYPE, Jdl.JOBTYPE_MPICH)) {
		    if (hasAttribute(Jdl.CPUNUMB)) {
		        throw new JobAdException(Jdl.CPUNUMB + ": attribute cannot be specified with non MPICH jobs");
		    }
		    if (hasAttribute(Jdl.NODENUMB)) {
		        throw new JobAdException(Jdl.NODENUMB + ": attribute cannot be specified with non MPICH jobs");
		    }
		}		
